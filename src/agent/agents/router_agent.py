...
class RouterAgent:
    ...
    INTENT_KEYWORDS = {
        "qa": [
            "怎么", "如何", "什么", "为什么", "能不能", "支持",
            "设置", "连接", "说明书", "参数", "规格", "功能", "使用",
            "定时", "清扫模式", "地图", "电压", "噪音", "尘盒",
        ],
        "troubleshoot": [
            "不工作", "坏了", "故障", "错误", "异常", "不动", "不转",
            "没反应", "无法", "不了", "连不上", "掉线", "开不了机",
            "E0", "E1", "错误码", "一直响", "指示灯", "卡住",
        ],
        "consumables": [
            "换", "买", "购", "耗材", "边刷", "滤网", "主刷", "拖布",
            "配件", "多少钱", "价格", "哪里买", "原装", "第三方", "套装",
        ],
    }
    ...
    async def route(self, state: dict) -> dict:
        ...
        3. 根据意图分流：
           ...
           - consumables → 耗材管理推荐
        """
        from src.agent.persona import is_pure_greeting, get_greeting_reply, is_out_of_scope, OUT_OF_SCOPE_REJECTION
        ...
        if task.get("diagnosis_stage") == "diagnosis":
            ...
            is_short_answer = len(query) <= 10 or \
                any(query.startswith(kw) for kw in ["是", "对", "有", "嗯", "不", "没", "亮", "能", "可以"])
            if is_short_answer:
    ...
    async def _is_diagnosis_answer(self, query: str, state: dict) -> bool:
        ...
        tree = None
        if fault_type:
            from src.app.scenarios.troubleshoot_scenario import DIAGNOSIS_TREE
