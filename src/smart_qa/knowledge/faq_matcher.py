...
class FAQMatcher:
    ...
    def match(self, query: str, threshold: float | None = None) -> str | None:
        ...
        for i, (faq_q, faq_tokens) in enumerate(zip(self._questions, self._question_tokens, strict=False)):
        ...
        if best_idx >= 0 and best_score >= threshold:
            logger.info("FAQMatcher 命中 score=%.2f threshold=%.2f query=%s", best_score, threshold, query[:60])
