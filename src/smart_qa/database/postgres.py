...
from smart_qa.models import ConsumableOrder, DeviceUsageLog, Session, UserDevice
...
class PostgresClient:
    ...
    async def get_user_device(db: AsyncSession, user_id: str) -> UserDevice | None:
        result = await db.execute(
            select(UserDevice)
            .where(UserDevice.user_id == user_id, UserDevice.is_active)
    ...
    async def bind_device(
        ...
        old = await db.execute(select(UserDevice).where(UserDevice.user_id == user_id, UserDevice.is_active))
    ...
    async def get_usage_stats(db: AsyncSession, user_id: str, days: int = 30) -> dict:
        ...
        return {
            ...
            "total_area": round(sum(log.clean_area for log in logs), 1),
            "total_duration": round(sum(log.duration_minutes for log in logs), 1),
            "avg_area_per_clean": round(sum(log.clean_area for log in logs) / len(logs), 1),
